...
async function fetchMessagesFromDB(docId: string) {
  ...
  const chats = await adminDb
    ...
    // .limit(6)
    .get();
  const chatHistory = chats.docs.map((doc: FirebaseFirestore.DocumentData) => {
  ...
  console.log(
    chatHistory.map((msg: HumanMessage | AIMessage) => msg.content.toString())
  );
  return chatHistory;
}
...
export async function generateEmbeddingsInPineconeVectorStore(
  ...
  // remove this one since we are going to pass the embeddings from others functions
